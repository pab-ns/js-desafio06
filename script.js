...
// 1.- AGREGAR
traumatologia.push(
    {hora: '9:00',  especialista: 'RENÉ POBLETE', paciente: 'ANA GELLONA', rut: '13123329-7', prevision: 'ISAPRE'},
...
// 2.- ELIMAR PRIMER Y ÚLTIMO ARREGLO RADIOLOGIA
radiologia.shift()
radiologia.pop()
console.log(radiologia)

// 3.- SEPARAR POR GUION Y POR PARRAFO
// document.write(`${dental.join(" - ")}`);

dental.forEach(infoPaciente => {
    document.write(`${infoPaciente.hora} - ${infoPaciente.especialista} - ${infoPaciente.paciente} - ${infoPaciente.rut} - ${infoPaciente.prevision} <br>`)
})

// 4.- IMPRIMIR LISTADO TODOS PACIENTES
document.write(`<br> Radiologia <br>`)
...
// 5.- FILTRAR ISAPRE DENTAL
document.write(`<br> Dental / ISAPRE <br>`)

let filterDental = dental.filter(dental =>{
...
// 6.- FILTAR TRAUMATOLOGIA FONASA
document.write(`<br> Traumatologia / FONASA <br>`)

let filterTrauma = traumatologia.filter(trauma =>{
    return trauma.prevision == "FONASA";
})

filterTrauma.forEach(paciente =>{
    document.write(`${paciente.paciente} - ${paciente.prevision} <br>`);
})
